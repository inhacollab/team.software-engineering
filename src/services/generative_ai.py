...
4. Output should be a **JSON array of location suggestions**, like: [{"location": "Inspire Island","description": "A peaceful waterfront resort with pools, relaxing lounges, and stunning architecture — perfect for unwinding and taking memorable photos."},{"location": "Jayu Park","description": "A beautiful hillside park with cherry blossoms, ocean views, and a peaceful walking trail — great for a reflective stroll."}]. There should not be any extra text or explanations, just the JSON array.
    ...
    try:
        ...
        if response and response.text:
            ...
            if lines[0].strip() == "```json":
                lines = lines[1:]  # Remove the opening fence
            if lines[-1].strip() == "```":
                lines = lines[:-1]  # Remove the closing fence

            # Join the lines back into a single string
            json_string = "\n".join(lines)

            # Parse the cleaned JSON string into a Python object (list of dictionaries)
